...
struct Vertex
{
    ...
    // Comparators to compare vertices by x, y and z coordinates
    struct ByX
    {
...
struct CubicTexture
{
    ...
    void createCubicTextureImageView();
    void createCubicTextureSampler();

    void init(BaseProject *bp, std::vector<std::string> files);
    void cleanup();
};
...
struct Pipeline
{
    ...
    void init(BaseProject *bp, const std::string &VertShader, const std::string &FragShader,
              std::vector<DescriptorSetLayout *> D, VkCompareOp compareOp, VkCullModeFlagBits cullMode);
...
public:
    ...
    ObjectInstance(ObjectType type, glm::vec3 pos, glm::vec3 rotation, glm::vec3 scale) : type(type),
                                                                                          position(pos),
                                                                                          rotation(rotation),
                                                                                          scale(scale){};
...
public:
    ...
    Object(std::string model, std::string texture, float defaultScale) : modelFile(model),
                                                                         textureFile(texture),
...
struct SkyBoxModel
{
    ...
    SkyBoxModel(std::string model, std::vector<std::string> textures) : modelFile(model),
...
struct Text
{
    ...
    Text(std::string modelFile, std::string textureFile, glm::vec3 pos, glm::vec3 rotation, glm::vec3 scale) : modelFile(modelFile),
...
protected:
    ...
    void createDepthResources()
    {
        ...
        depthImageView = createImageView(depthImage, depthFormat,
                                         VK_IMAGE_ASPECT_DEPTH_BIT, 1, VK_IMAGE_VIEW_TYPE_2D);
    }
...
{
    const auto [minX, maxX] = std::minmax_element(vertices.begin(), vertices.end(), Vertex::ByX());

    boundaries.minX = std::abs((*minX).pos.x);
    boundaries.maxX = std::abs((*maxX).pos.x);

    const auto [minY, maxY] = std::minmax_element(vertices.begin(), vertices.end(), Vertex::ByY());

    boundaries.minY = std::abs((*minY).pos.y);
    boundaries.maxY = std::abs((*maxY).pos.y);

    const auto [minZ, maxZ] = std::minmax_element(vertices.begin(), vertices.end(), Vertex::ByZ());

    boundaries.minZ = std::abs((*minZ).pos.z);
    boundaries.maxZ = std::abs((*maxZ).pos.z);

    std::cout << "Object min size (x, y, z) = (" << boundaries.minX << ", " << boundaries.minY << ", " << boundaries.minZ << ")" << std::endl;
    std::cout << "Object max size (x, y, z) = (" << boundaries.maxX << ", " << boundaries.maxY << ", " << boundaries.maxZ << ")" << std::endl
              << std::endl;
}
...
void Texture::createTextureImageView()
{
    textureImageView = BP->createImageView(textureImage,
                                           ...
                                           mipLevels, VK_IMAGE_VIEW_TYPE_2D);
}
                         ...
                         std::vector<DescriptorSetElement> E)
{
    ...
    for (size_t i = 0; i < BP->swapChainImages.size(); i++)
    {
        ...
        for (int j = 0; j < E.size(); j++)
        {
            ...
            else if (E[j].type == SKYBOX)
            {
                VkDescriptorImageInfo imageInfo{};
                imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                imageInfo.imageView = E[j].cubTex->textureImageView;
                imageInfo.sampler = E[j].cubTex->textureSampler;
                imageInfoVector.push_back(imageInfo);
...
void Object::init(BaseProject *bp)
{
    model.init(bp, modelFile);
    texture.init(bp, textureFile);
}
...
void ObjectInstance::init(BaseProject *bp, DescriptorSetLayout *L, std::vector<DescriptorSetElement> E)
{
    descSet.init(bp, L, E);
}
...
void SkyBoxModel::init(BaseProject *bp, DescriptorSetLayout *L, std::vector<DescriptorSetElement> E)
{
    if (textureFiles.size() != SKYBOX_TEXTURES)
    {
        throw std::runtime_error("invalid skybox textures!");
    }

    box.init(bp, modelFile);
    texture.init(bp, textureFiles);
    descSet.init(bp, L, E);
}

void SkyBoxModel::cleanup()
{
    descSet.cleanup();
    texture.cleanup();
    box.cleanup();
...
void CubicTexture::createCubicTextureImage(std::vector<std::string> files)
{
    int texWidth, texHeight, texChannels;
    stbi_uc *pixels[6];
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        std::cout << files[i] << " -> size: " << texWidth
...
void CubicTexture::createCubicTextureImageView()
{
    textureImageView = BP->createImageView(textureImage,
                                           VK_FORMAT_R8G8B8A8_SRGB,
                                           VK_IMAGE_ASPECT_COLOR_BIT,
...
void CubicTexture::init(BaseProject *bp, std::vector<std::string> files)
{
    BP = bp;
    createCubicTextureImage(files);
    createCubicTextureImageView();
...
void Text::init(BaseProject *bp, DescriptorSetLayout *L, std::vector<DescriptorSetElement> E)
{
    model.init(bp, modelFile);
    texture.init(bp, textureFile);
    descSet.init(bp, L, E);
